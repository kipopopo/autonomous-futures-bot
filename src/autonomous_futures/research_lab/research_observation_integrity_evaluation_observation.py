...
from .research_observation_integrity_evaluation_handoff import (
    ResearchObservationIntegrityEvaluationHandoff,
)


class ResearchObservationIntegrityEvaluationObservationInput(DomainModel):
    """Non-authoritative audit-only input for an integrity-evaluation observer."""
    ...
    def prepared_at_is_utc(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() != timedelta(0):
            raise ValueError(
                "research integrity evaluation observation input prepared_at must be "
                "timezone-aware UTC"
            )
        return value.astimezone(UTC)

    @model_validator(mode="after")
    def validate_hash(self) -> ResearchObservationIntegrityEvaluationObservationInput:
        if (
            research_observation_integrity_evaluation_observation_content_hash(self)
            != self.input_hash
        ):
            raise ValueError("research integrity evaluation observation input hash mismatch")
        return self


def research_observation_integrity_evaluation_observation_content_hash(
...
def build_research_observation_integrity_evaluation_observation_input(
    handoff: ResearchObservationIntegrityEvaluationHandoff,
    *,
    prepared_at: datetime | None = None,
) -> ResearchObservationIntegrityEvaluationObservationInput:
    """Accept only a hash-verified integrity-evaluation handoff."""
    try:
        verified_handoff = ResearchObservationIntegrityEvaluationHandoff.model_validate(
            handoff.model_dump()
        )
    except ValueError as exc:
        raise DomainViolation("research integrity evaluation handoff hash mismatch") from exc
    ...
    provisional = ResearchObservationIntegrityEvaluationObservationInput.model_construct(
        ...
        source_review_hash=verified_handoff.source_review_hash,
        source_evaluation_input_hash=verified_handoff.source_evaluation_input_hash,
        source_observation_hash=verified_handoff.source_observation_hash,
        check_count=verified_handoff.check_count,
