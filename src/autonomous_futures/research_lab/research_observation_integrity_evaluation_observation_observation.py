...
from .research_observation_integrity_evaluation_observation_result_handoff import (
...
class ResearchObservationIntegrityEvaluationObservationObservationInput(DomainModel):
    ...
    def valid_hash(
        ...
        if (
            research_observation_integrity_evaluation_observation_observation_content_hash(self)
...
def research_observation_integrity_evaluation_observation_observation_content_hash(
    o: ResearchObservationIntegrityEvaluationObservationObservationInput,
) -> str:
    return sha256(
        json.dumps(
...
def build_research_observation_integrity_evaluation_observation_observation_input(
    handoff: ResearchObservationIntegrityEvaluationObservationHandoff,
    *,
    observed_at: datetime | None = None,
) -> ResearchObservationIntegrityEvaluationObservationObservationInput:
    try:
        ResearchObservationIntegrityEvaluationObservationHandoff.model_validate(
            handoff.model_dump()
        )
