...
from .research_run_observation import ResearchObservationInput


class ResearchObservationEvaluationInput(DomainModel):
    """Non-authoritative input limited to deterministic audit-integrity review."""
    ...
    def prepared_at_is_utc(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() != timedelta(0):
            raise ValueError(
                "research observation evaluation prepared_at must be timezone-aware UTC"
            )
        return value.astimezone(UTC)

    @model_validator(mode="after")
    def validate_hash(self) -> ResearchObservationEvaluationInput:
        if research_observation_evaluation_content_hash(self) != self.evaluation_input_hash:
            raise ValueError("research observation evaluation input hash mismatch")
        return self


def research_observation_evaluation_content_hash(
    input_data: ResearchObservationEvaluationInput,
) -> str:
...
def build_research_observation_evaluation_input(
    observation: ResearchObservationInput,
    *,
    prepared_at: datetime | None = None,
) -> ResearchObservationEvaluationInput:
    """Accept only a hash-verified observation input for integrity review."""
    try:
        verified_observation = ResearchObservationInput.model_validate(observation.model_dump())
    except ValueError as exc:
        raise DomainViolation("research observation input hash mismatch") from exc
    ...
    provisional = ResearchObservationEvaluationInput.model_construct(
        ...
        audit_count=verified_observation.audit_count,
