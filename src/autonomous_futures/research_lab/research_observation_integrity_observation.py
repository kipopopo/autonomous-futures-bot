...
from .research_observation_integrity_handoff import ResearchObservationIntegrityHandoff


class ResearchObservationIntegrityObservationInput(DomainModel):
    """Non-authoritative audit-only input for an integrity observation consumer."""
    ...
    def prepared_at_is_utc(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() != timedelta(0):
            raise ValueError(
                "research integrity observation input prepared_at must be timezone-aware UTC"
            )
        return value.astimezone(UTC)

    @model_validator(mode="after")
    def validate_hash(self) -> ResearchObservationIntegrityObservationInput:
        if research_observation_integrity_observation_content_hash(self) != self.input_hash:
            raise ValueError("research integrity observation input hash mismatch")
        return self


def research_observation_integrity_observation_content_hash(
    input_data: ResearchObservationIntegrityObservationInput,
) -> str:
...
def build_research_observation_integrity_observation_input(
    handoff: ResearchObservationIntegrityHandoff,
    *,
    prepared_at: datetime | None = None,
) -> ResearchObservationIntegrityObservationInput:
    """Accept only a hash-verified integrity handoff as downstream input."""
    try:
        verified_handoff = ResearchObservationIntegrityHandoff.model_validate(handoff.model_dump())
    except ValueError as exc:
        raise DomainViolation("research observation integrity handoff hash mismatch") from exc
