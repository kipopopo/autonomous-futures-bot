"""Phase 287: Production Canary Full Autonomous Multi-Candidate Cross-Asset Order Book Depth
Imbalance Runner, Asymmetric Liquidity Evaporation Governance & Stepped Exposure Scaling.

Implements the deterministic Phase 287 autonomous execution daemon runner, cross-asset
order book depth imbalance tracking, adaptive maker spread cushioning, asymmetric liquidity
evaporation governance, stepped exposure scaling up to 40.00 USDT, aggregate margin headroom
...
DEFAULT_PHASE286_DIR: Path = DEFAULT_PHASE286_OUTPUT_DIR
...
AGGREGATE_CONCURRENT_EXPOSURE_CAP_USDT: Decimal = Decimal("40.00")
...
# Depth Imbalance & Queue Depletion Governance Thresholds (Phase 287)
NOMINAL_IMBALANCE_THRESHOLD: Decimal = Decimal("0.30")  # |I_depth| <= 0.30 is nominal
ELEVATED_IMBALANCE_THRESHOLD: Decimal = Decimal("0.60")  # |I_depth| > 0.60 triggers severe controls
QUEUE_DEPLETION_TOLERANCE: Decimal = Decimal("0.50")  # 50% queue drop triggers elevated controls
QUEUE_DEPLETION_RISK_THRESHOLD: Decimal = Decimal("0.60")  # > 60% queue drop triggers throttling
THROTTLED_PER_CANDIDATE_CAP_USDT: Decimal = Decimal("10.00")

# Backward-compatible aliases for prior phase thresholds
NOMINAL_SHOCK_THRESHOLD: Decimal = NOMINAL_IMBALANCE_THRESHOLD
ELEVATED_SHOCK_THRESHOLD: Decimal = ELEVATED_IMBALANCE_THRESHOLD
DEPTH_DEPLETION_TOLERANCE: Decimal = QUEUE_DEPLETION_TOLERANCE
...
TRACK_DESCRIPTIONS: dict[str, str] = {
    "track_1": (
        ...
        "(Nominal depth imbalance tracking, spread monitoring across BTCUSDT, "
        "ETHUSDT, SOLUSDT -> parallel lifecycle management -> clean ledger updates)"
    ),
    "track_2": (
        "Asymmetric Depth Collapse & Adverse Selection Throttling Drill "
        "(Simulate extreme order book skew -> dynamic child order downscaling, "
        "limit offset widening, and fail-closed dispatch rejection on carry risk boundaries)"
    ),
    "track_3": (
        "Cross-Asset Liquidity Evaporation & Circuit Breaker Liquidation Drill "
...
class CanaryDepthImbalanceError(DomainViolation):
    """Base exception for Phase 287 depth imbalance runner operations."""


CanaryLiquidityShockError = CanaryDepthImbalanceError
CanaryVolatilitySpilloverError = CanaryDepthImbalanceError


class PrerequisiteQualificationError(
    UpstreamPrerequisiteQualificationError, CanaryDepthImbalanceError
):
    """Raised when upstream qualification or certification is missing or invalid."""


class IndividualMicroCapExceededError(CanaryDepthImbalanceError):
    """Raised when order notional exceeds 5.00 USDT individual micro order cap."""


class MicroNotionalFloorViolationError(CanaryDepthImbalanceError):
    """Raised when order notional falls below 1.00 USDT micro floor."""


class AggregateExposureCapExceededError(CanaryDepthImbalanceError):
    """Raised when concurrent active exposure exceeds active stage expansion cap."""


class MarginAllocationExceededError(CanaryDepthImbalanceError):
    """Raised when margin allocation exceeds per-asset (20%) or aggregate (60%) ceiling."""


class CashReserveBufferBreachedError(CanaryDepthImbalanceError):
    """Raised when unencumbered cash reserve buffer falls below 40% requirement."""


class IntraPhaseLossCeilingExceededError(CanaryDepthImbalanceError):
    """Raised when cumulative intra-phase loss exceeds 5.00 USDT loss ceiling."""


class GatewayHeartbeatStaleError(CanaryDepthImbalanceError):
...
class InvalidClientOrderIdTagError(CanaryDepthImbalanceError):
    """Raised when client order ID does not conform to canary deterministic tagging."""


class CircuitBreakerAbortError(CanaryDepthImbalanceError):
    """Raised when order dispatch is attempted while circuit breaker is tripped."""


class OrderCorrelationError(CanaryDepthImbalanceError):
    """Raised when order lifecycle transition fails correlation or causality check."""


class ListenKeyLifecycleError(CanaryDepthImbalanceError):
...
class DepthExhaustionError(CanaryDepthImbalanceError):
    """Raised when order book depth is exhausted or below minimum safety threshold."""


class SpreadExceededError(CanaryDepthImbalanceError):
    """Raised when bid-ask spread exceeds maximum tolerable ceiling."""


class DepthImbalanceToleranceExceededError(CanaryDepthImbalanceError):
    """Raised when depth imbalance index exceeds tolerance boundary (|I_depth| > 0.60)."""


LiquidityShockToleranceExceededError = DepthImbalanceToleranceExceededError
...
FundingRateDistortionThrottledError = QueueDepletionThrottledError
CorrelationBreakdownThrottledError = QueueDepletionThrottledError


class AggressiveOrderRejectedError(QueueDepletionThrottledError):
    """Raised when aggressive order is rejected during queue depletion or depth collapse."""


class OrderSlicingError(CanaryDepthImbalanceError):
    """Raised when dynamic micro-order slicing cannot be completed safely."""


# Backward compatibility aliases
CryptographicVerificationError = CanaryDepthImbalanceError
InsufficientCashReserveError = CashReserveBufferBreachedError
StreamDisconnectError = CanaryDepthImbalanceError
...
class DepthImbalanceRegime(StrEnum):
    """Cross-asset order book depth imbalance state classification."""

    NOMINAL = "NOMINAL"
    ELEVATED_IMBALANCE = "ELEVATED_IMBALANCE"
    SEVERE_CONTROLS = "SEVERE_CONTROLS"
    ELEVATED_SHOCK = "ELEVATED_IMBALANCE"


LiquidityShockRegime = DepthImbalanceRegime
VolatilitySpilloverRegime = DepthImbalanceRegime
LiquidityRegime = DepthImbalanceRegime


class QueueDepletionRiskState(StrEnum):
    """Queue depletion / evaporation risk status across canary candidates."""

    NORMAL = "NORMAL"
    ELEVATED = "ELEVATED"
    SEVERE = "SEVERE"
    MODERATE_DIVERGENCE = "ELEVATED"
    EXTREME_DISTORTION = "SEVERE"


FundingRateDistortionState = QueueDepletionRiskState
CorrelationState = QueueDepletionRiskState


class CanaryDepthImbalanceTrackId(StrEnum):
    """Identifiers for the 4 Phase 287 simulation tracks."""
...
CanaryLiquidityShockTrackId = CanaryDepthImbalanceTrackId
CanaryVolatilitySpilloverTrackId = CanaryDepthImbalanceTrackId
CanaryLiquidityRegimeTrackId = CanaryDepthImbalanceTrackId
CanaryAdaptiveExecutionTrackId = CanaryDepthImbalanceTrackId
CanaryContinuousDaemonTrackId = CanaryDepthImbalanceTrackId
...
CANARY_CLIENT_ORDER_ID_REGEX = re.compile(
    r"^c=canary-p287-(?P<symbol>[A-Z0-9]+)-(?P<timestamp>\d+)-(?P<uuid>[a-zA-Z0-9\-]+)$"
)
...
def generate_canary_client_order_id(
    ...
    return f"c=canary-p287-{sym}-{ts}-{uid}"
...
def validate_canary_client_order_id(
    ...
    if not match:
        return (
            ...
            r"^c=canary-p287-{symbol}-{timestamp}-{uuid}$",
        )
...
class DepthImbalanceSnapshot(DomainModel):
    """Snapshot of cross-asset depth imbalance, depletion transmission, and queue risk."""

    snapshot_id: str = Field(default_factory=lambda: uuid4().hex[:12])
    timestamp_utc: str = Field(default_factory=lambda: datetime.now(UTC).isoformat())
    track_id: str
    aggregate_imbalance_index: str = "0.0"
    regime: DepthImbalanceRegime = DepthImbalanceRegime.NOMINAL
    btc_depth_imbalance: str = "0.0"
    eth_depth_imbalance: str = "0.0"
    sol_depth_imbalance: str = "0.0"
    btc_depth_depletion: str = "0.0"
    eth_depth_depletion: str = "0.0"
    sol_depth_depletion: str = "0.0"
    queue_depletion_risk: str = "0.0"
    depth_imbalance_active: bool = False
    # Backward-compatible fields
    shock_index: str = "0.0"
    btc_funding_rate: str = "0.0001"
    eth_funding_rate: str = "0.00015"
    sol_funding_rate: str = "0.0002"
    max_funding_basis_spread: str = "0.0"
    funding_distortion_active: bool = False


LiquidityShockSnapshot = DepthImbalanceSnapshot
VolatilitySpilloverSnapshot = DepthImbalanceSnapshot
...
class DepthImbalanceOrderRecord(DomainModel):
    """Order record for Phase 287."""
...
LiquidityShockOrderRecord = DepthImbalanceOrderRecord
VolatilityOrderRecord = DepthImbalanceOrderRecord
...
LiquidityShockDaemonTrackResult = DepthImbalanceDaemonTrackResult
VolatilityDaemonTrackResult = DepthImbalanceDaemonTrackResult
...
# Compatibility alias
CanaryLiquidityShockReport = CanaryDepthImbalanceReport
CanaryVolatilitySpilloverReport = CanaryDepthImbalanceReport
...
class CanaryDepthImbalanceConfig(DomainModel):
    ...
    output_dir: Path = DEFAULT_PHASE287_OUTPUT_DIR
    track: str = "all"
...
# Compatibility alias
CanaryLiquidityShockConfig = CanaryDepthImbalanceConfig
CanaryVolatilitySpilloverConfig = CanaryDepthImbalanceConfig
...
class SqliteCanaryDepthImbalanceTelemetryStore:
    ...
    def _init_schema(self) -> None:
        with self._lock, self.conn:
            self.conn.executescript(
                ...
                CREATE TABLE IF NOT EXISTS depth_imbalance_snapshots (
                    snapshot_id TEXT PRIMARY KEY,
                    timestamp_utc TEXT NOT NULL,
                    track_id TEXT NOT NULL,
                    aggregate_imbalance_index TEXT NOT NULL,
                    regime TEXT NOT NULL,
                    btc_depth_imbalance TEXT NOT NULL,
                    eth_depth_imbalance TEXT NOT NULL,
                    sol_depth_imbalance TEXT NOT NULL,
                    btc_depth_depletion TEXT NOT NULL,
                    eth_depth_depletion TEXT NOT NULL,
                    sol_depth_depletion TEXT NOT NULL,
                    queue_depletion_risk TEXT NOT NULL,
                    depth_imbalance_active INTEGER NOT NULL
                );
    ...
    def record_depth_imbalance_snapshot(self, snap: DepthImbalanceSnapshot) -> None:
        with self._lock, self.conn:
            self.conn.execute(
                """
                INSERT OR REPLACE INTO depth_imbalance_snapshots (
                    snapshot_id, timestamp_utc, track_id, aggregate_imbalance_index, regime,
                    btc_depth_imbalance, eth_depth_imbalance, sol_depth_imbalance,
                    btc_depth_depletion, eth_depth_depletion, sol_depth_depletion,
                    queue_depletion_risk, depth_imbalance_active
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    snap.snapshot_id,
                    snap.timestamp_utc,
                    snap.track_id,
                    getattr(snap, "aggregate_imbalance_index", "0.0"),
                    snap.regime.value,
                    getattr(snap, "btc_depth_imbalance", "0.0"),
                    getattr(snap, "eth_depth_imbalance", "0.0"),
                    getattr(snap, "sol_depth_imbalance", "0.0"),
                    snap.btc_depth_depletion,
                    snap.eth_depth_depletion,
                    snap.sol_depth_depletion,
                    getattr(snap, "queue_depletion_risk", "0.0"),
                    1 if getattr(snap, "depth_imbalance_active", False) else 0,
                ),
            )

    def record_shock_snapshot(self, snap: DepthImbalanceSnapshot) -> None:
        self.record_depth_imbalance_snapshot(snap)

    record_spillover_snapshot = record_depth_imbalance_snapshot
...
# Compatibility alias
SqliteCanaryLiquidityShockTelemetryStore = SqliteCanaryDepthImbalanceTelemetryStore
SqliteCanaryVolatilitySpilloverTelemetryStore = SqliteCanaryDepthImbalanceTelemetryStore
...
class DepthImbalanceEngine:
    ...
    def get_depth_imbalance(self, symbol: str) -> Decimal:
        """Calculate normalized top-of-book depth imbalance:
        I_depth = (V_bid - V_ask) / (V_bid + V_ask)
        Returns Decimal in [-1.0, 1.0]. If total depth is zero, returns 0.0.
        """
        with self._lock:
            sym_key = str(symbol).strip().upper()
            ...
            if total_v <= Decimal("0.0"):
                return Decimal("0.0")
            imb = (v_bid - v_ask) / total_v
            return max(
                Decimal("-1.0"),
                min(Decimal("1.0"), imb.quantize(Decimal("0.0001"), rounding=ROUND_DOWN)),
            )

    def get_queue_depletion_risk(self, symbol: str | None = None) -> Decimal:
        """Calculate queue depletion / evaporation risk ratio across candidates."""
        with self._lock:
            if symbol is not None:
                return self.get_depth_depletion_ratio(symbol)
            ratios = [self.get_depth_depletion_ratio(s) for s in CANARY_STAGED_SYMBOLS]
            return max(ratios) if ratios else Decimal("0.0")

    def is_depth_imbalance_exceeded(self, symbol: str | None = None) -> bool:
        """Check whether order book depth imbalance tolerance boundary (|I_depth| > 0.60)
        or queue depletion risk is breached.
        """
        with self._lock:
            if self.funding_distortion_forced:
                return True
            if symbol is not None:
                imb = abs(self.get_depth_imbalance(symbol))
                if imb > ELEVATED_IMBALANCE_THRESHOLD:
                    return True
                if self.get_depth_depletion_ratio(symbol) > QUEUE_DEPLETION_RISK_THRESHOLD:
                    return True
            else:
                for s in CANARY_STAGED_SYMBOLS:
                    if abs(self.get_depth_imbalance(s)) > ELEVATED_IMBALANCE_THRESHOLD:
                        return True
                    if self.get_depth_depletion_ratio(s) > QUEUE_DEPLETION_RISK_THRESHOLD:
                        return True
            return False
    ...
    def classify_depth_imbalance_regime(self, symbol: str | None = None) -> DepthImbalanceRegime:
        ...
        with self._lock:
            ...
            # 1. Severe depth imbalance or queue depletion risk
            severe_condition = (
                self.is_depth_imbalance_exceeded(sym_key)
                or self.is_funding_distortion(sym_key)
                or self.aggregate_shock_index > ELEVATED_IMBALANCE_THRESHOLD
                or (
                    sym_key
                    and abs(self.get_depth_imbalance(sym_key)) > ELEVATED_IMBALANCE_THRESHOLD
                )
                or (
                    sym_key
    ...
    classify_spillover_regime = classify_depth_imbalance_regime
...
# Compatibility alias
LiquidityShockEngine = DepthImbalanceEngine
VolatilitySpilloverEngine = DepthImbalanceEngine
...
# Compatibility alias
MockBinanceLiquidityShockGateway = MockBinanceDepthImbalanceGateway
MockBinanceVolatilityGateway = MockBinanceDepthImbalanceGateway
MockBinanceVolatilitySpilloverGateway = MockBinanceDepthImbalanceGateway
...
class DepthImbalanceStreamSequencer:
...
# Compatibility alias
LiquidityShockStreamSequencer = DepthImbalanceStreamSequencer
VolatilityStreamSequencer = DepthImbalanceStreamSequencer
VolatilitySpilloverStreamSequencer = DepthImbalanceStreamSequencer


class DepthUserDataStreamReconciler:
...
# Compatibility alias
LiquidityUserDataStreamReconciler = DepthUserDataStreamReconciler
VolatilityUserDataStreamReconciler = DepthUserDataStreamReconciler
...
class DepthImbalanceOrderDispatchInterlock:
    ...
    def __init__(
        ...
        self.shock_engine = shock_engine or DepthImbalanceEngine()
        self.depth_engine = self.shock_engine

    # Compatibility alias
    @property
    def spillover_engine(self) -> DepthImbalanceEngine:
        return self.shock_engine

    @spillover_engine.setter
    def spillover_engine(self, engine: DepthImbalanceEngine) -> None:
...
# Compatibility alias
LiquidityShockOrderDispatchInterlock = DepthImbalanceOrderDispatchInterlock
VolatilityOrderDispatchInterlock = DepthImbalanceOrderDispatchInterlock
VolatilitySpilloverOrderDispatchInterlock = DepthImbalanceOrderDispatchInterlock
...
class DepthMicroOrderDispatcher:
...
# Compatibility alias
LiquidityMicroOrderDispatcher = DepthMicroOrderDispatcher
VolatilityMicroOrderDispatcher = DepthMicroOrderDispatcher
...
class DepthImbalanceAutonomousDaemon:
...
# Compatibility alias
LiquidityShockAutonomousDaemon = DepthImbalanceAutonomousDaemon
VolatilityAutonomousDaemon = DepthImbalanceAutonomousDaemon
VolatilitySpilloverAutonomousDaemon = DepthImbalanceAutonomousDaemon
...
def verify_upstream_phase286_qualification(
    ...
    """Verify upstream Phase 286 liquidity shock report, prerequisites, and DAG hash chain."""
    p286_path = Path(phase286_dir)
    try:
    ...
    summary_file = p286_path / "liquidity-shock-summary.json"
    report_file = p286_path / "canary-liquidity-shock-report.json"

    if not summary_file.is_file():
        raise PrerequisiteQualificationError(
            f"Phase 286 liquidity shock summary missing at {summary_file}"
        )
    if not report_file.is_file():
        raise PrerequisiteQualificationError(
            f"Phase 286 canary liquidity shock report missing at {report_file}"
        )
    ...
    sum_status = sum_data.get("liquidity_shock_status") or sum_data.get("daemon_status")
    if sum_status != "LIQUIDITY_SHOCK_VERIFIED":
        raise PrerequisiteQualificationError(
            f"Phase 286 status is {sum_status}, expected LIQUIDITY_SHOCK_VERIFIED"
        )
    ...
    rep_status = rep_data.get("liquidity_shock_status") or rep_data.get("daemon_status")
    if rep_status != "LIQUIDITY_SHOCK_VERIFIED":
        raise PrerequisiteQualificationError(
            f"Phase 286 report status is {rep_status}, expected LIQUIDITY_SHOCK_VERIFIED"
        )

    # 2. Check compliance flags
    comp = sum_data.get("compliance", {})
    if not comp.get("all_criteria_passed"):
        raise PrerequisiteQualificationError("Phase 286 compliance all_criteria_passed is False")
    if not comp.get("zero_balance_drift"):
        raise PrerequisiteQualificationError("Phase 286 compliance zero_balance_drift is False")
    if not comp.get("liquidity_shock_transmission_verified"):
        raise PrerequisiteQualificationError(
            "Phase 286 compliance liquidity_shock_transmission_verified is False"
        )
    ...
    for sym in CANARY_STAGED_SYMBOLS:
        if sym not in candidates:
            raise PrerequisiteQualificationError(
                f"Candidate {sym} missing from Phase 286 candidates"
            )

    # 4. Verify continuous hash chain back through Phase 286 to Phase 276
    chain_ok = verify_phase_286_hash_chain(
        output_dir=p286_path,
    ...
    if not chain_ok:
        raise PrerequisiteQualificationError("Phase 286 Merkle DAG hash chain verification failed")

    return True


# Backward compatibility aliases
verify_upstream_phase285_qualification = verify_upstream_phase286_qualification
verify_upstream_phase284_qualification = verify_upstream_phase286_qualification
...
class CanaryDepthImbalanceRunner:
    ...
    def execute_all_tracks(self) -> CanaryLiquidityShockReport:
        ...
        db_path = self.output_dir / "canary-depth-imbalance-telemetry.sqlite3"
        ...
        report_data: dict[str, Any] = {
            "phase": "phase_287",
            "description": (
                "Phase 287 Production Canary Full Autonomous Multi-Candidate Cross-Asset "
                "Order Book Depth Imbalance Runner Report"
            ),
            "timestamp_utc": now_utc,
            "daemon_status": "DEPTH_IMBALANCE_VERIFIED",
            ...
            "artifact_hashes": {
                ...
                "canary-depth-imbalance-telemetry.sqlite3": actual_db_hash,
            },
        }

        # Write report JSON
        report_path = self.output_dir / "canary-depth-imbalance-report.json"
        report_bytes = canonical_json_bytes(report_data)
        assert_zero_secrets(report_bytes.decode("utf-8"), "canary-depth-imbalance-report.json")
        ...
        summary_data: dict[str, Any] = {
            "phase": "phase_287",
            "description": (
                "Phase 287 Production Canary Multi-Candidate Cross-Asset Order Book Depth "
                "Imbalance Runner Summary"
            ),
            "timestamp_utc": now_utc,
            "daemon_status": "DEPTH_IMBALANCE_VERIFIED",
            "manifest_version": manifest.manifest_version,
            ...
            "artifact_hashes": {
                "canary-orders.jsonl": actual_jsonl_hash,
                "canary-depth-imbalance-telemetry.sqlite3": actual_db_hash,
                "canary-depth-imbalance-report.json": actual_report_hash,
            },
        }

        summary_path = self.output_dir / "depth-imbalance-summary.json"
        summary_bytes = canonical_json_bytes(summary_data)
        assert_zero_secrets(summary_bytes.decode("utf-8"), "depth-imbalance-summary.json")
        ...
        paper_summary_data: dict[str, Any] = {
            "phase": "phase_287",
            "description": (
                "Phase 287 Production Canary Multi-Candidate "
                "Cross-Asset Order Book Depth Imbalance Paper Summary"
            ),
            "timestamp_utc": now_utc,
            "manifest_version": manifest.manifest_version,
            "staged_manifest_hash": manifest.manifest_hash,
            ...
            "artifact_hashes": {
                ...
                "canary-depth-imbalance-telemetry.sqlite3": actual_db_hash,
                "canary-depth-imbalance-report.json": actual_report_hash,
                "depth-imbalance-summary.json": actual_summary_hash,
            },
        }
    ...
    def _run_track_1(
        ...
        result = LiquidityShockDaemonTrackResult(
            ...
            status="SUCCESS_DEPTH_IMBALANCE_EXECUTION_AND_FILL_RECONCILED",
    ...
    def _run_track_2(
        ...
        # 3. Simulate Depth Imbalance Collapse Drill:
        # BTC skews heavily (|I_depth| > 0.60): bid 0.00001 vs ask 0.00020 -> I_depth = -0.9048
        shock_engine.update_book(
            "BTCUSDT",
            bid_price=Decimal("60000.00"),
        ...
        assert shock_engine.is_depth_imbalance_exceeded("BTCUSDT") is True
        ...
        result = LiquidityShockDaemonTrackResult(
            ...
            status="SUCCESS_DEPTH_COLLAPSE_AND_THROTTLING_VERIFIED",
            starting_equity_usdt=str(reconciler.starting_equity),
            final_cash_usdt=str(reconciler.cash),
            allocated_margin_usdt=str(reconciler.allocated_margin),
            unrealized_pnl_usdt=str(reconciler.unrealized_pnl),
    ...
    def _run_track_3(
        ...
        # BTCUSDT: 2 x 0.00005 @ 60,000 = 6.00 USDT (each <= 5.00 USDT cap)
        # ETHUSDT: 0.0015 @ 3,000 = 4.50 USDT (<= 5.00 USDT cap)
        hb_data2a1 = gateway.generate_heartbeat(latency_ms=25.0)
        hb_rec2a1 = heartbeat_mon.record_heartbeat(
            server_time_ms=hb_data2a1["serverTime"],
            latency_ms=hb_data2a1["latencyMs"],
            track_id=CanaryLiquidityShockTrackId.TRACK_3.value,
        )
        self.active_store.record_heartbeat(hb_rec2a1)
        ...
        hb_data2a2 = gateway.generate_heartbeat(latency_ms=25.0)
        hb_rec2a2 = heartbeat_mon.record_heartbeat(
            server_time_ms=hb_data2a2["serverTime"],
            latency_ms=hb_data2a2["latencyMs"],
            track_id=CanaryLiquidityShockTrackId.TRACK_3.value,
        )
        self.active_store.record_heartbeat(hb_rec2a2)
        ...
        # Realized loss = 0.00010 * (60,000 - 2,000) = 5.80 USDT > 5.00 USDT ceiling!
        hb_data3 = gateway.generate_heartbeat(latency_ms=25.0)
        hb_rec3 = heartbeat_mon.record_heartbeat(
            server_time_ms=hb_data3["serverTime"],
            latency_ms=hb_data3["latencyMs"],
...
# Compatibility alias
CanaryLiquidityShockRunner = CanaryDepthImbalanceRunner
CanaryVolatilitySpilloverRunner = CanaryDepthImbalanceRunner
...
def verify_phase_287_hash_chain(
    output_dir: Path | str = DEFAULT_PHASE287_OUTPUT_DIR,
    manifest_path: Path | str = DEFAULT_CANARY_STAGING_MANIFEST_PATH,
    phase276_dir: Path | str = DEFAULT_PHASE276_OUTPUT_DIR,
    phase277_dir: Path | str = DEFAULT_PHASE277_OUTPUT_DIR,
    phase278_dir: Path | str = DEFAULT_PHASE278_OUTPUT_DIR,
    ...
    """Verify cryptographic SHA-256 DAG hash chain and balance integrity for Phase 287."""
    out_dir = Path(output_dir)
    manifest, _ = load_and_validate_canary_staging_manifest(Path(manifest_path))

    jsonl_path = out_dir / "canary-orders.jsonl"
    db_path = out_dir / "canary-depth-imbalance-telemetry.sqlite3"
    report_path = out_dir / "canary-depth-imbalance-report.json"
    summary_path = out_dir / "depth-imbalance-summary.json"
    ...
    for p in [jsonl_path, db_path, report_path, summary_path, paper_summary_path]:
        if not p.is_file():
            logger.error("Missing required Phase 287 artifact: %s", p)
            return False
    ...
    # 2. Verify Upstream Phase 286 back to 276
    p286_path = Path(phase286_dir)
    if not p286_path.is_dir():
        logger.error("Upstream Phase 286 directory not found: %s", p286_path)
        return False
    if not verify_upstream_phase286_qualification(
        phase286_dir=p286_path,
        manifest_path=manifest_path,
        phase276_dir=phase276_dir,
        phase277_dir=phase277_dir,
        phase278_dir=phase278_dir,
        ...
        logger.error("Upstream Phase 286 hash chain / qualification verification failed")
        return False
    ...
    # 3. Verify report data integrity
    try:
    ...
    if rep_hashes.get("canary-orders.jsonl") != actual_jsonl_hash:
        ...
        return False
    if rep_hashes.get("canary-depth-imbalance-telemetry.sqlite3") != actual_db_hash:
        logger.error("Report canary-depth-imbalance-telemetry.sqlite3 hash mismatch")
        return False

    # 4. Verify depth-imbalance-summary.json
    try:
    ...
    if sum_hashes.get("canary-depth-imbalance-report.json") != actual_report_hash:
        logger.error("Summary canary-depth-imbalance-report.json hash mismatch")
        return False
    if sum_hashes.get("canary-depth-imbalance-telemetry.sqlite3") != actual_db_hash:
        logger.error("Summary canary-depth-imbalance-telemetry.sqlite3 hash mismatch")
        return False
    ...
    if paper_hashes.get("canary-depth-imbalance-report.json") != actual_report_hash:
        logger.error("Paper summary canary-depth-imbalance-report.json hash mismatch")
        return False
    if paper_hashes.get("depth-imbalance-summary.json") != actual_summary_hash:
        logger.error("Paper summary depth-imbalance-summary.json hash mismatch")
        return False
    if paper_hashes.get("canary-depth-imbalance-telemetry.sqlite3") != actual_db_hash:
        logger.error("Paper summary canary-depth-imbalance-telemetry.sqlite3 hash mismatch")
        return False
    if paper_hashes.get("canary-orders.jsonl") != actual_jsonl_hash:
        logger.error("Paper summary canary-orders.jsonl hash mismatch")
        return False

    # 6. Verify zero balance drift in compliance
    if not report_data.get("compliance", {}).get("zero_balance_drift"):
        logger.error("Report compliance zero_balance_drift is False")
        return False
    if not sum_data.get("compliance", {}).get("zero_balance_drift"):
        logger.error("Summary compliance zero_balance_drift is False")
        return False
    if not paper_data.get("zero_balance_drift"):
        logger.error("Paper summary zero_balance_drift is False")
        return False

    return True


verify_phase_286_hash_chain_alias = verify_phase_287_hash_chain

__all__ = [
    "DEFAULT_PHASE287_OUTPUT_DIR",
    "DEFAULT_PHASE286_DIR",
    "DEFAULT_PHASE287_DIR",
    "MIN_MICRO_NOTIONAL_CAP_USDT",
    "HARD_MICRO_NOTIONAL_CAP_USDT",
    "DYNAMIC_SLICING_MAX_CHUNK_USDT",
    ...
    "LISTEN_KEY_REFRESH_INTERVAL_SECONDS",
    "SEQUENCE_WRAP_THRESHOLD",
    "NOMINAL_IMBALANCE_THRESHOLD",
    "ELEVATED_IMBALANCE_THRESHOLD",
    "QUEUE_DEPLETION_TOLERANCE",
    "QUEUE_DEPLETION_RISK_THRESHOLD",
    "THROTTLED_PER_CANDIDATE_CAP_USDT",
    "MIN_REQUIRED_BOOK_DEPTH",
    "MAX_TOLERABLE_SPREAD_PCT",
    "DEFAULT_DEPTH_EXHAUSTION_THRESHOLD",
    "TRACK_DESCRIPTIONS",
    "CanaryDepthImbalanceError",
    ...
    "QueueDepletionRiskState",
    "CanaryDepthImbalanceTrackId",
    ...
    "WebSocketPushEvent",
    "DaemonLifecycleEvent",
    "DepthImbalanceDaemonTrackResult",
    "CanaryDepthImbalanceReport",
    ...
    "DepthImbalanceAutonomousDaemon",
