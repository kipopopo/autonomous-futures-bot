...
from autonomous_futures.research_lab.research_observation_integrity_evaluation_observation_observation_handoff_observation_result_input import (
    load_verified_research_observation_integrity_evaluation_observation_observation_handoff_observation_review,
)


def _invalid():
    return ResearchObservationIntegrityEvaluationObservationObservationHandoffObservationInput.model_construct(
...
def test_loader_rejects_invalid_observation_hash(tmp_path: Path):
...
def test_loader_rejects_missing_after_validated_caller(tmp_path: Path):
    p = _invalid()
    p.observation_hash = "0" * 64
    with pytest.raises(DomainViolation):
        load_verified_research_observation_integrity_evaluation_observation_observation_handoff_observation_review(
            tmp_path / "missing.json", observation=p
        )
