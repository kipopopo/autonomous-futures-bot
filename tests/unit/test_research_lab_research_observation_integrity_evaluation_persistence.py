...
def _review() -> ResearchObservationIntegrityEvaluationReview:
...
def test_integrity_evaluation_review_persistence_round_trips_verified_review(
    tmp_path: Path,
) -> None:
    review = _review()
    path = tmp_path / "reviews" / "research-run-0001.json"

    assert write_research_observation_integrity_evaluation_review(path, review) == review
    assert read_research_observation_integrity_evaluation_review(path) == review


def test_integrity_evaluation_review_persistence_is_idempotent_and_write_once(
    tmp_path: Path,
) -> None:
    review = _review()
    ...
    with pytest.raises(DomainViolation, match="immutable"):
        write_research_observation_integrity_evaluation_review(path, changed)


def test_integrity_evaluation_review_reader_rejects_bad_artifacts(tmp_path: Path) -> None:
    review = _review()
    path = tmp_path / "research-run-0001.json"
    ...
    with pytest.raises(DomainViolation, match="hash mismatch"):
        read_research_observation_integrity_evaluation_review(path)

    malformed = tmp_path / "malformed.json"
    malformed.write_text("{not-json", encoding="utf-8")
    with pytest.raises(DataQualityError, match="invalid persisted"):
        read_research_observation_integrity_evaluation_review(malformed)
    with pytest.raises(FileNotFoundError):
        read_research_observation_integrity_evaluation_review(tmp_path / "missing.json")


def test_integrity_evaluation_review_writer_rejects_hash_mismatch_before_filesystem_work(
    tmp_path: Path,
) -> None:
    path = tmp_path / "new" / "research-run-0001.json"
    invalid = _review().model_copy(update={"review_hash": "0" * 64})

    with pytest.raises(DomainViolation, match="hash mismatch"):
        write_research_observation_integrity_evaluation_review(path, invalid)
    assert not path.parent.exists()


def test_integrity_evaluation_review_writer_cleans_temp_file_on_link_failure(
    tmp_path: Path,
    ...
    monkeypatch.setattr(
        "autonomous_futures.research_lab.research_observation_integrity_evaluation_persistence.os.link",
        fail_link,
    )
