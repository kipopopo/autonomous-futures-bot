...
**Decision: LOCAL RELEASE GATES PENDING FINAL FULL SUITE; no deployment performed.**
...
- A previously committed marker remains durable when the state write fails, when marker clearing fails after a successful state write, and across a fresh SQLite reopen. The old state is never accepted while the marker remains.
...
- GREEN: focused Phase 265–269 recovery/accounting set: `18 passed in 2.24s`.
...
- Final acceptance requires the locked full pytest run, commit, push to `origin/main`, and exact remote SHA readback.
- Parent review of this durability implementation is required before any remote write or deployment. If final gates fail, the tree must be reported blocked and not deployed.
