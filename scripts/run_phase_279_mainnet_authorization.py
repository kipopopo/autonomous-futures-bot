"""Phase 279: Production Canary Live Mainnet Micro-Execution Authorization Runner CLI.

Validates production canary live mainnet micro-execution authorization harness,
...
logger = logging.getLogger("run_phase_279_mainnet_authorization")


def build_arg_parser() -> argparse.ArgumentParser:
    """Construct CLI argument parser for Phase 279 runner."""
    parser = argparse.ArgumentParser(
        description=(
            "Phase 279 Production Canary Live Mainnet Micro-Execution Authorization Harness CLI."
        )
    )
    parser.add_argument(
        "--manifest-path",
    ...
    parser.add_argument(
        ...
        help="Path to output canary mainnet telemetry artifacts",
    )
    parser.add_argument(
        "--track",
        type=str,
...
def format_summary_table(report: CanaryMainnetReport) -> str:
    """Format Phase 279 mainnet authorization results into human-readable ASCII table."""
    lines: list[str] = [
        "=" * 92,
...
def execute_phase_279_runner(
    ...
    output_dir: Path = DEFAULT_PHASE279_OUTPUT_DIR,
    track: str = "all",
    ...
    """Execute deterministic Phase 279 mainnet authorization runner workflow."""
    if verify_only:
        hash_ok = verify_phase_279_hash_chain(
        ...
        sys.stdout.write(
            "[PASS] Cryptographic SHA-256 DAG hash chain verified across Phase 279 artifacts.\n"
        )
        return 0
...
def main(argv: list[str] | None = None) -> int:
    """Main CLI entry point for Phase 279 runner script."""
    ...
    try:
        return execute_phase_279_runner(
        ...
        logger.info("Mainnet authorization runner cancelled by operator")
        return 1
    except Exception as exc:
        logger.error("Mainnet authorization runner failed: %s", exc, exc_info=True)
        return 1
