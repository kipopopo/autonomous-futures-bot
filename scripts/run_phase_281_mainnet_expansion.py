"""Phase 281: Production Canary Live Mainnet Staged Capital Expansion Runner CLI.

Validates production canary live mainnet staged capital expansion runner,
...
logger = logging.getLogger("run_phase_281_mainnet_expansion")


def build_arg_parser() -> argparse.ArgumentParser:
    """Construct CLI argument parser for Phase 281 runner."""
    ...
    parser.add_argument(
        ...
        help="Path to output canary mainnet expansion telemetry artifacts",
    )
    parser.add_argument(
        "--track",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Intra-phase cumulative loss ceiling in USDT (default: 2.00)",
    )
    parser.add_argument(
...
def format_summary_table(report: CanaryMainnetExpansionReport) -> str:
    """Format Phase 281 mainnet expansion results into human-readable ASCII table."""
    lines: list[str] = [
        "=" * 92,
...
def execute_phase_281_runner(
    ...
    """Execute deterministic Phase 281 mainnet expansion runner workflow."""
    if verify_only:
        hash_ok = verify_phase_281_hash_chain(
        ...
        sys.stdout.write(
            "[PASS] Cryptographic SHA-256 DAG hash chain verified across Phase 281 artifacts.\n"
        )
        return 0
...
def main(argv: list[str] | None = None) -> int:
    """Main CLI entry point for Phase 281 runner script."""
    ...
    try:
        return execute_phase_281_runner(
        ...
        logger.info("Mainnet expansion runner cancelled by operator")
        return 1
    except Exception as exc:
        logger.error("Mainnet expansion runner failed: %s", exc, exc_info=True)
        return 1
