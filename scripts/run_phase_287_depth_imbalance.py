"""Phase 287: Production Canary Order Book Depth Imbalance & Exposure Scaling Runner CLI.

Validates production canary multi-candidate autonomous continuous live execution daemon runner,
cross-asset order book depth imbalance tracking, asymmetric liquidity evaporation governance,
stepped exposure scaling up to 40.00 USDT, and continuous balance reconciliation across staged
canary symbols (BTCUSDT, ETHUSDT, SOLUSDT) under Candidate Registry Manifest Version 2.
"""
...
logger = logging.getLogger("run_phase_287_depth_imbalance")


def build_arg_parser() -> argparse.ArgumentParser:
    """Construct CLI argument parser for Phase 287 depth imbalance runner."""
    parser = argparse.ArgumentParser(
        description=(
            "Phase 287 Production Canary Multi-Candidate Cross-Asset Order Book Depth "
            "Imbalance Transmission Autonomous Daemon Runner CLI."
        )
    )
    parser.add_argument(
        "--manifest-path",
    ...
    parser.add_argument(
        ...
        help="Path to output canary depth imbalance telemetry artifacts",
    )
    parser.add_argument(
        "--track",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Intra-phase cumulative loss ceiling in USDT (default: 5.00)",
    )
    parser.add_argument(
...
def format_summary_table(report: CanaryDepthImbalanceReport) -> str:
    """Format Phase 287 depth imbalance results into human-readable ASCII table."""
    lines: list[str] = [
        "=" * 96,
        "   PHASE 287: PRODUCTION CANARY DEPTH IMBALANCE & EXPOSURE SCALING",
        "=" * 96,
...
def execute_phase_287_runner(
    ...
    """Execute deterministic Phase 287 depth imbalance runner workflow."""
    if verify_only:
        hash_ok = verify_phase_287_hash_chain(
        ...
        sys.stdout.write(
            "[PASS] Cryptographic SHA-256 DAG hash chain verified across Phase 287 artifacts.\n"
        )
        return 0
    ...
    all_tracks_ok = all(t.get("success", False) for t in report.tracks) and report.compliance.get(
...
def main(argv: list[str] | None = None) -> int:
    """Main CLI entry point for Phase 287 runner script."""
    ...
    try:
        return execute_phase_287_runner(
        ...
        logger.info("Depth imbalance runner cancelled by operator")
        return 1
    except Exception as exc:
        logger.error("Depth imbalance runner failed: %s", exc, exc_info=True)
        return 1
