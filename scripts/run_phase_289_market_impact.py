"""Phase 289: Production Canary Microstructural Market Impact & Exposure Scaling Runner CLI.

Validates production canary multi-candidate autonomous continuous live execution daemon runner,
cross-asset microstructural market impact tracking, Kyle's Lambda (λ = ΔP / Q) price impact
governance, stepped exposure scaling up to 50.00 USDT, and continuous balance reconciliation
...
from autonomous_futures.feed.market_impact import (  # noqa: E402
    ...
    CanaryMarketImpactConfig,
    CanaryMarketImpactReport,
    CanaryMarketImpactRunner,
...
logger = logging.getLogger("run_phase_289_market_impact")


def build_arg_parser() -> argparse.ArgumentParser:
    """Construct CLI argument parser for Phase 289 market impact runner."""
    parser = argparse.ArgumentParser(
        description=(
            "Phase 289 Production Canary Multi-Candidate Cross-Asset Microstructural "
            "Market Impact & Exposure Scaling Autonomous Daemon Runner CLI."
        )
    )
    parser.add_argument(
        "--manifest-path",
    ...
    parser.add_argument(
        ...
        help="Path to output canary market impact telemetry artifacts",
    )
    parser.add_argument(
        "--track",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Intra-phase cumulative loss ceiling in USDT (default: 6.00)",
    )
    parser.add_argument(
...
def format_summary_table(report: CanaryMarketImpactReport) -> str:
    """Format Phase 289 market impact results into human-readable ASCII table."""
    lines: list[str] = [
        "=" * 96,
        "   PHASE 289: PRODUCTION CANARY MICROSTRUCTURAL MARKET IMPACT & EXPOSURE SCALING",
        "=" * 96,
...
def execute_phase_289_runner(
    ...
    """Execute deterministic Phase 289 market impact runner workflow."""
    if verify_only:
        hash_ok = verify_phase_289_hash_chain(
        ...
        sys.stdout.write(
            "[PASS] Cryptographic SHA-256 DAG hash chain verified across Phase 289 artifacts.\n"
        )
        return 0
    ...
    runner = CanaryMarketImpactRunner(cfg)
...
def main(argv: list[str] | None = None) -> int:
    """Main CLI entry point for Phase 289 runner script."""
    ...
    try:
        return execute_phase_289_runner(
        ...
        logger.info("Market impact runner cancelled by operator")
        return 1
    except Exception as exc:
        logger.error("Market impact runner failed: %s", exc, exc_info=True)
        return 1
