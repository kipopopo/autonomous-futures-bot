"""Phase 284: Production Canary Liquidity Regime Shifting & Dynamic Order Slicing Runner CLI.

Validates production canary multi-candidate autonomous continuous live execution daemon runner,
dynamic liquidity regime shifting, dynamic order slicing governance, stepped exposure scaling up
to 25.00 USDT, and continuous balance reconciliation across staged canary symbols
...
logger = logging.getLogger("run_phase_284_liquidity_regime")


def build_arg_parser() -> argparse.ArgumentParser:
    """Construct CLI argument parser for Phase 284 liquidity regime runner."""
    parser = argparse.ArgumentParser(
        description=(
            "Phase 284 Production Canary Liquidity Regime Shifting & Dynamic "
            "Order Slicing Autonomous Daemon Runner CLI."
        )
    )
    parser.add_argument(
        "--manifest-path",
    ...
    parser.add_argument(
        ...
        help="Path to output canary liquidity regime telemetry artifacts",
    )
    parser.add_argument(
        "--track",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Intra-phase cumulative loss ceiling in USDT (default: 3.50)",
    )
    parser.add_argument(
...
def format_summary_table(report: CanaryLiquidityRegimeReport) -> str:
    """Format Phase 284 liquidity regime results into human-readable ASCII table."""
    lines: list[str] = [
        "=" * 96,
        "   PHASE 284: PRODUCTION CANARY LIQUIDITY REGIME SHIFTING & DYNAMIC SLICING",
        "=" * 96,
...
def execute_phase_284_runner(
    ...
    """Execute deterministic Phase 284 liquidity regime runner workflow."""
    if verify_only:
        ...
        sys.stdout.write(
            "[PASS] Cryptographic SHA-256 DAG hash chain verified across Phase 284 artifacts.\n"
        )
        return 0
...
def main(argv: list[str] | None = None) -> int:
    """Main CLI entry point for Phase 284 runner script."""
    ...
    try:
        return execute_phase_284_runner(
        ...
        logger.info("Liquidity regime runner cancelled by operator")
        return 1
    except Exception as exc:
        logger.error("Liquidity regime runner failed: %s", exc, exc_info=True)
        return 1
