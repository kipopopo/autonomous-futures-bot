"""Phase 288: Production Canary Order Flow Toxicity & Exposure Scaling Runner CLI.

Validates production canary multi-candidate autonomous continuous live execution daemon runner,
cross-asset order flow toxicity tracking, Volume-Synchronized Probability of Toxicity (VPIN)
divergence governance, stepped exposure scaling up to 45.00 USDT, and continuous balance
...
logger = logging.getLogger("run_phase_288_flow_toxicity")


def build_arg_parser() -> argparse.ArgumentParser:
    """Construct CLI argument parser for Phase 288 flow toxicity runner."""
    parser = argparse.ArgumentParser(
        description=(
            "Phase 288 Production Canary Multi-Candidate Cross-Asset Order Flow "
            "Toxicity & Pacing Autonomous Daemon Runner CLI."
        )
    )
    parser.add_argument(
        "--manifest-path",
    ...
    parser.add_argument(
        ...
        help="Path to output canary flow toxicity telemetry artifacts",
    )
    parser.add_argument(
        "--track",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Intra-phase cumulative loss ceiling in USDT (default: 5.50)",
    )
    parser.add_argument(
...
def format_summary_table(report: CanaryFlowToxicityReport) -> str:
    """Format Phase 288 flow toxicity results into human-readable ASCII table."""
    lines: list[str] = [
        "=" * 96,
        "   PHASE 288: PRODUCTION CANARY ORDER FLOW TOXICITY & EXPOSURE SCALING",
        "=" * 96,
...
def execute_phase_288_runner(
    ...
    """Execute deterministic Phase 288 flow toxicity runner workflow."""
    if verify_only:
        hash_ok = verify_phase_288_hash_chain(
        ...
        sys.stdout.write(
            "[PASS] Cryptographic SHA-256 DAG hash chain verified across Phase 288 artifacts.\n"
        )
        return 0
    ...
    runner = CanaryFlowToxicityRunner(cfg)
...
def main(argv: list[str] | None = None) -> int:
    """Main CLI entry point for Phase 288 runner script."""
    ...
    try:
        return execute_phase_288_runner(
        ...
        logger.info("Flow toxicity runner cancelled by operator")
        return 1
    except Exception as exc:
        logger.error("Flow toxicity runner failed: %s", exc, exc_info=True)
        return 1
