"""Phase 290: Production Canary OFI & Cross-Impact Matrix Price Transmission Runner CLI.

Validates production canary multi-candidate autonomous continuous live execution daemon runner,
cross-asset Order Flow Imbalance (OFI), rolling cross-impact matrix (Γ_ij), lead-lag adverse
selection prevention, stepped exposure scaling up to 55.00 USDT, and continuous balance
reconciliation across staged canary symbols (BTCUSDT, ETHUSDT, SOLUSDT) under Candidate Registry
Manifest Version 2.
"""
...
from autonomous_futures.feed.ofi_cross_impact import (  # noqa: E402
    ...
    CanaryOfiCrossImpactConfig,
    CanaryOfiCrossImpactReport,
    CanaryOfiCrossImpactRunner,
...
logger = logging.getLogger("run_phase_290_ofi_cross_impact")


def build_arg_parser() -> argparse.ArgumentParser:
    """Construct CLI argument parser for Phase 290 OFI and cross-impact runner."""
    parser = argparse.ArgumentParser(
        description=(
            "Phase 290 Production Canary Multi-Candidate Cross-Asset Order Flow Imbalance "
            "& Cross-Impact Matrix Autonomous Daemon Runner CLI."
        )
    )
    parser.add_argument(
        "--manifest-path",
    ...
    parser.add_argument(
        ...
        help="Path to output canary OFI & cross-impact telemetry artifacts",
    )
    parser.add_argument(
        "--track",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Intra-phase cumulative loss ceiling in USDT (default: 6.50)",
    )
    parser.add_argument(
...
def format_summary_table(report: CanaryOfiCrossImpactReport) -> str:
    """Format Phase 290 OFI & cross-impact results into human-readable ASCII table."""
    lines: list[str] = [
        "=" * 96,
        "   PHASE 290: PRODUCTION CANARY OFI & CROSS-IMPACT MATRIX (Gamma_ij) PRICE TRANSMISSION",
        "=" * 96,
        ...
        f"Upstream P285 Rep: {report.upstream_phase285_report_hash[:16]}...",
        f"Upstream P286 Rep: {report.upstream_phase286_report_hash[:16]}...",
        f"Upstream P287 Rep: {report.upstream_phase287_report_hash[:16]}...",
        f"Upstream P288 Rep: {report.upstream_phase288_report_hash[:16]}...",
        f"Upstream P289 Rep: {report.upstream_phase289_report_hash[:16]}...",
        "-" * 96,
        f"{'Track ID':<10} {'Status':<48} {'Drift (USDT)':<15} {'Success':<8}",
        "-" * 96,
    ]
    for tr in report.tracks:
        tid = tr.get("track_id", "") if isinstance(tr, dict) else getattr(tr, "track_id", "")
        st = tr.get("status", "") if isinstance(tr, dict) else getattr(tr, "status", "")
        dr = (
            tr.get("drift_usdt", "0.0")
            if isinstance(tr, dict)
            else getattr(tr, "drift_usdt", "0.0")
        )
        sc = tr.get("success", False) if isinstance(tr, dict) else getattr(tr, "success", False)
        lines.append(f"{tid:<10} {st:<48} {dr:<15} {str(sc):<8}")
    lines.extend(
        [
            "-" * 96,
...
def execute_phase_290_runner(
    ...
    phase285_dir: Path = DEFAULT_PHASE285_OUTPUT_DIR,
    phase286_dir: Path = DEFAULT_PHASE286_OUTPUT_DIR,
    phase287_dir: Path = DEFAULT_PHASE287_OUTPUT_DIR,
    phase288_dir: Path = DEFAULT_PHASE288_OUTPUT_DIR,
    phase289_dir: Path = DEFAULT_PHASE289_OUTPUT_DIR,
    ...
    """Execute deterministic Phase 290 OFI and cross-impact runner workflow."""
    if verify_only:
        hash_ok = verify_phase_290_hash_chain(
        ...
        sys.stdout.write(
            "[PASS] Cryptographic SHA-256 DAG hash chain verified across Phase 290 artifacts.\n"
        )
        return 0
    ...
    cfg = CanaryOfiCrossImpactConfig(
    ...
    runner = CanaryOfiCrossImpactRunner(cfg)
...
def main(argv: list[str] | None = None) -> int:
    """Main CLI entry point for Phase 290 runner script."""
    ...
    try:
        return execute_phase_290_runner(
        ...
        logger.info("OFI & cross-impact runner cancelled by operator")
        return 1
    except Exception as exc:
        logger.error("OFI & cross-impact runner failed: %s", exc, exc_info=True)
        return 1
