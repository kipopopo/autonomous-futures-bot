"""Phase 268: Multi-Week Paper Trading Cohort Observation Scaling & Long-Horizon Maturation.
...
persistence, deterministic multi-week observation scaling (14 days / 1,344 15m / 4,032 5m bars),
periodic 6-hour fixed-slot observation marks into paper-observations.sqlite3 with strict
deduplication guards, single-position invariants across shared 100 USDT margin portfolio,
automated cohort readiness evaluation, exact double-entry accounting reconciliation (<1e-15 drift),
and strict fail-closed containment.
"""
...
logger = logging.getLogger("run_phase_268_long_horizon_maturation")

DEFAULT_PHASE268_OUTPUT_DIR = Path("artifacts/research/phase268")
...
# 14 canonical days: 14 * 24 * 12 = 4,032 5m bars;
# 14 * 24 * 4 = 1,344 15m bars; 14 * 4 = 56 6-hour slots
DEFAULT_OBSERVATION_DAYS = 14.0
DEFAULT_OBSERVATION_TICKS = 4032
DEFAULT_CANONICAL_15M_BARS = 1344
DEFAULT_CANONICAL_6H_SLOTS = 56
...
async def replay_long_horizon_cohort_observations(
    engine: LivePaperTradingEngine,
    history_dir: Path,
    symbols: Sequence[str],
    max_ticks: int = DEFAULT_OBSERVATION_TICKS,
    stop_event: asyncio.Event | None = None,
) -> int:
    """Replay deterministic multi-week bars and tickers sequentially from canonical parquet data.
    ...
    if not symbol_dfs:
        logger.warning("No canonical data found for long-horizon observation replay")
        return 0
    ...
    for ts in target_timestamps:
        if stop_event is not None and stop_event.is_set():
            logger.info(
                "Stop event set; terminating long-horizon observation replay early at tick %d",
                ticks_processed,
            )
            break
    ...
    logger.info(
        "Completed long-horizon replay of %d bar ticks across %d symbols",
        ticks_processed,
        len(symbols),
    )
...
replay_multi_day_cohort_observations = replay_long_horizon_cohort_observations


@dataclass(frozen=True, slots=True)
class Phase268LongHorizonMaturationResult:
    """Complete telemetry result of Phase 268 long-horizon cohort maturation run."""
...
Phase268CohortMaturationResult = Phase268LongHorizonMaturationResult


async def run_phase_268_long_horizon_maturation(
    *,
    output_dir: Path = DEFAULT_PHASE268_OUTPUT_DIR,
    ...
    required_days: int = 7,
) -> Phase268LongHorizonMaturationResult:
    """Execute deterministic Phase 268 long-horizon maturation runner under Manifest Version 2."""
    ...
    # Determine replay bar count: default to 14 canonical days (4,032 5m bars)
    if ticks is not None:
        ...
        batch_ticks_count = DEFAULT_OBSERVATION_TICKS
    ...
    logger.info(
        "Starting Phase 268 paper long-horizon maturation runner "
        "(mode=%s, duration=%.1fs, ticks=%s)",
        effective_mode,
        session_duration,
    ...
    try:
        ...
        if effective_mode == "batch":
            ticks_processed = await replay_long_horizon_cohort_observations(
                engine=engine,
                history_dir=history_dir,
                symbols=symbols,
            ...
            if stream_failed and not stop_event.is_set():
                logger.warning(
                    "Live public stream unavailable or empty (%s); executing long-horizon replay",
    ...
    summary_payload: dict[str, Any] = {
        "phase": "phase_268",
        "description": (
            "Phase 268 Multi-Week Cohort Observation Scaling & Long-Horizon Maturation Evaluation "
            "(Manifest v2)"
        ),
        ...
        "observation_horizon": {
            "canonical_days": DEFAULT_OBSERVATION_DAYS,
            "canonical_5m_bars": DEFAULT_OBSERVATION_TICKS,
            "canonical_15m_bars": DEFAULT_CANONICAL_15M_BARS,
            "canonical_6h_slots": DEFAULT_CANONICAL_6H_SLOTS,
            "replayed_ticks": ticks_processed,
            "replayed_days": round(ticks_processed / 288, 4),
        },
        "candidates": {
            sym: {
        ...
        "maturation_progression": {
            ...
            "per_symbol_maturation": {
                sym: {
                    ...
                    "trades_count": sum(1 for e in closed_entries if e.symbol == sym),
                    "sample_size": len(cand_obs_map.get(sym, [])),
                    "observed_slots": len(cand_obs_map.get(sym, [])),
    ...
    logger.info("Phase 268 cohort maturation successfully completed! Cash: %s USDT", final_cash)

    return Phase268LongHorizonMaturationResult(
        output_dir=output_dir,
        registry_version=manifest.registry_version,
        registry_hash=manifest.registry_hash,
        ...
        replay_days=round(ticks_processed / 288, 4),
    )


# Backward-compatible alias
run_phase_268_cohort_maturation = run_phase_268_long_horizon_maturation


def build_arg_parser() -> argparse.ArgumentParser:
    """Build CLI argument parser for Phase 268 long-horizon maturation runner."""
    parser = argparse.ArgumentParser(
        description=(
            "Phase 268: Multi-Week Paper Trading Cohort Observation Scaling & Maturation Evaluation"
        )
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_PHASE268_OUTPUT_DIR,
        help=(
            "Directory to persist SQLite ledgers and audit reports "
            "(default: artifacts/research/phase268)"
        ),
    )
    parser.add_argument(
        "--registry-path",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Observation replay duration in days (default: 14.0 / 4,032 5m bars / 1,344 15m bars)",
    )
    parser.add_argument(
        "--ticks",
        type=int,
        default=None,
        help="Number of 5m bar ticks to process in batch mode (default: 4,032 / 14 canonical days)",
    )
    parser.add_argument(
        "--smoke-test",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        default=7,
        help="Required observation days for maturity evaluation (default: 7)",
    )
    parser.add_argument(
        "--log-level",
        type=str,
...
def main(argv: list[str] | None = None) -> int:
    """Entry point for Phase 268 long-horizon maturation runner."""
    ...
    try:
        ...
        sys.stdout.write("\n=== PHASE 268 LONG-HORIZON MATURATION RUNNER COMPLETED ===\n")
        ...
        sys.stdout.write(
            f"Observation Horizon:    {res.replay_days:.2f} days ({res.ticks_processed} ticks)\n"
        )
        ...
        logger.info("Long-horizon maturation runner cancelled by operator")
        return 0
    except Exception as exc:
        logger.error("Long-horizon maturation runner failed with exception: %s", exc, exc_info=True)
        return 1
